...
const addPalette = function (parent: SvgElement, props: ModuleProperties): {
	g: SvgGElement,
	...
	const g: SvgGElement = parent.append('g')
		.datum(datumG)
...
const addLogo = function (parent: SvgElement, props: ModuleProperties): SvgImageElement | undefined {
	...
	if (props.icon) {
		...
		const svgImage = parent.append('image')
			.attr('x', x)
			.attr('y', y)
			.attr('width', width)
			.attr('height', height)
