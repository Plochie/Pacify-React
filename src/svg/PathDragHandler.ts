 ...
 */
export const pathDFunction = d3.line<{ x: number, y: number }>()
...
const dragStart = function (this: Element, d: any) {
	...
	if (ownerGroup.getAttribute('class') === CLASS.ACTION.WRAPPER) {
		...
		tempPath = svgRoot.select(`#${CLASS.CONNECTION.ID}`).append('path')
		...
		tempPathPoint = svgRoot.append('rect')
			.data([{ x, y }])
...
export const connectPalettes = (fromIO: SVGElement, toIO: SVGElement) => {
	...
	const path = svgRoot.select(`#${CLASS.CONNECTION.ID}`).append('path')
...
const PathDragHandler = d3Drag.drag()
	.on('drag', dragging)
	.on('start', dragStart)
